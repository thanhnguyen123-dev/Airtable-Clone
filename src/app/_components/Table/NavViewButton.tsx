...
const NavViewButton = ( {name, viewId, currentView, setCurrentView} : NavViewButtonProps ) => {
  ...
  const isViewSelected = currentView === viewId;

  return (
    <div
      role="button"
      className={`flex items-center justify-between gap-2 py-[6px] px-2 rounded-md hover:bg-slate-200 ${isViewSelected ? 'bg-[#C4ECFFB3] hover:bg-[#C4ECFF]' : ''}`}
      onClick={handleClick}
    >
      ...
      {isViewSelected && (
        ...
        fill="currentColor"
        >
          <use href="icons/icons_definitions.svg#Check"></use>
        </svg>
      )}
    </div>
  );
