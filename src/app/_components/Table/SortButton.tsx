      ...
      <PopoverTrigger>
        <div 
          ...
          className={`flex justify-center items-center gap-1 rounded-md px-2 py-1
            ${(sortColumnId !== "" && sort !== "") ? "bg-[#FFF2EA]": "hover:bg-slate-200"}
          `}
        >
          <svg
            width={16}
          ...
          <span className='text-slate-600'>{(sortColumnId !== "" && sort !== "") ? `Sorted by ${columns?.find((col) => col.id === sortColumnId)?.name}` : "Sort"}</span>
        </div>
      </PopoverTrigger>
      <PopoverContent
        className="w-[450px]"
